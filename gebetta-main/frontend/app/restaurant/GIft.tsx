...
const SendGift: React.FC<{ setGiftLocation: (location: { lat: number, lng: number }) => void, giftLocation: { lat: number, lng: number } | null }> = ({ setGiftLocation, giftLocation }) => {
  ...
  const fetchSuggestions = async (query: string) => {
    try {
      ...
      // Restrict to Ethiopia (ET) and bound to Addis Ababa area via viewbox
      // Viewbox format: left,top,right,bottom (lon,lat)
      // Addis Ababa approx bounds: left=38.60, top=9.05, right=38.90, bottom=8.85
      const url = `https://nominatim.openstreetmap.org/search?format=json&addressdetails=1&limit=5&countrycodes=et&viewbox=38.60,9.05,38.90,8.85&bounded=1&q=${encodeURIComponent(
        query
      )}`;
      const res = await fetch(url, {
        headers: {
          "Accept-Language": "en",
          // Identifying UA is recommended by Nominatim usage policy
          "User-Agent": "GebetaApp/1.0 (contact: support@gebeta.example)",
        },
      });
  ...
  // ⏱️ Debounce address input for suggestions
  useEffect(() => {
  ...
  // 🧹 Build a clean, human-friendly label without house numbers
  const getSuggestionLabel = (s: { display_name: string }) => {
    try {
      const parts = s.display_name
        .split(",")
        .map((p) => p.trim())
        // remove parts that contain digits (house numbers, codes)
        .filter((p) => !/\d/.test(p));
      // Ensure Addis Ababa is present
      const hasAddis = parts.some((p) => /addis\s*ab+\w*a?/i.test(p) || /addis/i.test(p));
      let finalParts = parts.slice(0, 3);
      if (!hasAddis) {
        finalParts = ["Addis Ababa", ...finalParts].slice(0, 3);
      }
      const label = finalParts.join(", ");
      return label || s.display_name;
    } catch {
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        {isSuggesting && address.trim().length >= 3 ? (
              ...
              <TouchableOpacity
                ...
                onPress={() => {
                  ...
                  if (!isNaN(lat) && !isNaN(lon)) {
                    const label = getSuggestionLabel(s as any);
                    setAddress(label);
                    setCoords({ lat, lng: lon });
                    setGiftLocation({ lat, lng: lon });
                    setMapRegion({
                      latitude: lat,
                      longitude: lon,
                      latitudeDelta: 0.01,
